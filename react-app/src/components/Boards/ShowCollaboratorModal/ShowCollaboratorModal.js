            ...
            <ol id="collaborators-list">
                <div className='single-user-container-owner' onClick={() => { closeModal(); history.push(`/${owner.username}`); }}>
                    <div className="collaborator-user-image-container">
                        <img src={owner?.photo_url ? owner.photo_url : 'https://flavoreatsbucket.s3.us-west-2.amazonaws.com/profile-icon.jpeg'} alt={owner?.username} className="collaborator-user-image" />
                ...
                {collaborators?.map((user) => (
                    <li key={user.id} className='single-user-container-with-button' onClick={() => { closeModal(); history.push(`/${user.username}`); }}>
                        <div className='single-user-container'>
                            <div className="collaborator-user-image-container">
                                <img src={user.photo_url ? user.photo_url : 'https://flavoreatsbucket.s3.us-west-2.amazonaws.com/profile-icon.jpeg'} alt={user.username} className="collaborator-user-image" />
                            </div>
                            <p>{user.username}</p>
